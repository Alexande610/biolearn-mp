        ...
        <div className="max-w-4xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              {/* Energy */}
              <div className="flex items-center gap-2 bg-green-700/50 px-3 py-2 rounded-xl">
                <Leaf className="w-5 h-5 text-green-300 leaf-energy" />
                <span className="text-white font-bold">{userStats?.energy || 0}</span>
                <span className="text-green-300 text-sm">/{userStats?.max_energy || 20}</span>
                {(userStats?.energy || 0) < (userStats?.max_energy || 20) && (
                  <Link to="/minigame/6" className="ml-1">
                    <Plus className="w-4 h-4 text-green-300 hover:text-white" />
                  </Link>
                )}
              </div>

              {/* Coins */}
              <div className="flex items-center gap-2 bg-yellow-600/50 px-3 py-2 rounded-xl">
                <Coins className="w-5 h-5 text-yellow-300" />
                <span className="text-white font-bold">{userStats?.coins || 0}</span>
              </div>
            </div>
          </div>
        </div>
